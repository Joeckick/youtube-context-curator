...
from config import Channel, LOOKBACK_HOURS, SUPADATA_API_KEY
...
def get_new_videos(channel: Channel, lookback_hours: int = LOOKBACK_HOURS) -> list[Video]:
    """
    ...
    for entry in feed.entries:
        ...
        if not video_id:
            ...
            if "v=" in link:
                ...
                logger.warning(f"Could not extract video ID for {entry.get('title', 'unknown')}")
                continue
...
def fetch_transcript(video: Video) -> Video:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if video.transcript:
                logger.info(f"Successfully fetched transcript ({len(video.transcript)} chars)")
            else:
            ...
            video.transcript_error = "Transcript requires processing (not immediately available)"
            ...
            logger.error(f"Supadata API error for {video.title}: {response.status_code} - {response.text}")
    ...
    for channel in channels:
        ...
        for video in videos:
            ...
            if video.transcript and is_likely_short_by_transcript(video.transcript):
                logger.info(f"Skipping likely Short (transcript too short): {video.title}")
                continue
